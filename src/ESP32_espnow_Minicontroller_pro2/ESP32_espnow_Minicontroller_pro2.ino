...
//    
//      zero_test();    //启动一次自动纠偏，但此时大多没有将开发板装到遥控器上，所以数据不对
//      eeprom_broadcastAddress();     //存入广播模式MAC地址
//        
//      EEPROM.write(0, 55);
//      EEPROM.commit();
//    }
...
//
//  Serial.print("已储存并用来配对的MAC地址：");  
//  for (int i = 0; i < 6; i++) {
//    broadcastAddress[i]=EEPROM.read(MAC_ADDRESS_OFFSET + i);  
//    Serial.print(EEPROM.read(MAC_ADDRESS_OFFSET + i), HEX); // 以16进制格式打印MAC地址  
//    if (i < 5) {  
//      Serial.print(":"); // 每2个字节之间用"-"分隔  
//    }  
//  }  
//  Serial.println(); 
//  
//}
//
//void eeprom_broadcastAddress(){
//  for (int i = 0; i < 6; i++) {  
//    EEPROM.write(MAC_ADDRESS_OFFSET + i, broadcastAddress[i]);
//  }  
//}
